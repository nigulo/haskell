...
test_functionValidate = do
        ...
        let
            myFunc :: F.Function (Double) = F.function "x+(4))"
        assertEqual [(0, ("Unexpected closing bracket", 5))] (F.validate myFunc)

        let
            myFunc :: F.Function (Double) = F.function "x+(4)("
        assertEqual [(0, ("Missing closing bracket for opening bracket", 5))] (F.validate myFunc)

        let
            myFunc :: F.Function (Double) = F.function "x+((4)"
        assertEqual [(0, ("Missing closing bracket", 5))] (F.validate myFunc)

        let
            myFunc :: F.Function (Double) = F.function "x+()()"
        assertEqual [(0, ("Parse error", 2))] (F.validate myFunc)
