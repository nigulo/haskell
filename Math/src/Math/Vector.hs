...
--set i value (Vector v) = 
--    Vector $ h ++ (value:t) where
--        (h, (_:t)) = splitAt i v
...
--getLength (Vector v) = length v
getLength (Vector v) = u - l + 1 where
    (l, u) = bounds v

dotProduct :: Num a => Vector a -> Vector a -> a
--dotProduct (Vector v1@(v10:v1s)) (Vector (v20:v2s)) = 
--    if (length v1 == 0) then 1
--    else (v10 * v20) + ((Vector v1s) `dotProduct` (Vector v2s))
dotProduct (Vector v1) (Vector v2) =
    sum [v1 ! i * v2 ! i | i <- indices v1]

op :: Num a => (a -> a -> a) -> Vector a -> Vector a -> Vector a
--op f (Vector []) (Vector []) = Vector []
--op f (Vector (v1:v1s)) (Vector (v2:v2s)) = 
--    let Vector vs = op f (Vector v1s) (Vector v2s)
--    in Vector ((v1 `f` v1):vs)
