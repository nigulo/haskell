module Regression.PolynomTest (tests) where
...
tests :: TestTree
tests = testGroup "Polynom"
    [ testCase "constantOp" test_constantOp
    , testCase "binaryOp" test_binaryOp
    , testCase "polynomProduct" test_polynomProduct
    , testCase "polynomSum" test_polynomSum
    , testCase "getValues" test_getValues
    , testCase "getTangents" test_getTangents
    , testCase "getDerivatives" test_getDerivatives
    ]

test_constantOp :: Assertion
test_constantOp = do
    let
        f1 = function "sin(x)"
        d1 = function "cos(x)"
...
test_getValues = do
    let
        ...
        xVal = 5
        result = getValues xVal pol
        expectedResult = [[2 * sin xVal, 0.7 * sin xVal * xVal, 1.5 * sin xVal * xVal ^ 3]]
    result @?= expectedResult

test_getTangents :: Assertion
test_getTangents = do
    let
        f = function "sin(x)"
        d = function "cos(x)"
        pol = Polynom [([(0, 2), (1, 0.7), (3, 1.5)], Just f, Just d)]
        xVal = 5
        result = getTangents xVal pol
        expectedResult = [2 * cos xVal, 0.7 * (sin xVal + xVal * cos xVal), 1.5 * (3 * xVal ^ 2 * sin xVal + xVal ^ 3 * cos xVal)]
    length result @?= 1
    assertEqualDoubleList expectedResult (head result)

test_getDerivatives :: Assertion
test_getDerivatives = do
    let
        ...
        pol = Polynom [([(0, 2), (1, 0.7), (3, 1.5)], Just f, Just d)]
        xVal = 5
        result = getDerivatives 2 xVal pol
        expectedResult = [-2 * sin xVal, 0.7 * (cos xVal + cos xVal - xVal * sin xVal), 1.5 * (6 * xVal * sin xVal + 6 * xVal ^ 2 * cos xVal - xVal ^ 3 * sin xVal)]
    length result @?= 1
    assertEqualDoubleList expectedResult (head result)
