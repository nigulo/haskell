...
plotDialog stateRef = do
    ...
    Gtk.fileDialogSetTitle dialog "Export to GNU"

    fileFilter <- Gtk.fileFilterNew
    Gtk.fileFilterAddPattern fileFilter "*.EPS"
    Gtk.fileFilterAddPattern fileFilter "*.PNG"
...
paramsDialog stateRef = do
    ...
    titleEntry <- Gtk.entryNew
    addWidgetToBox (Just "Title: ") titleEntry hBox1
    entrySetText titleEntry (gnuTitle gnuParms)
    titleOptsEntry <- Gtk.entryNew
    addWidgetToBox Nothing titleOptsEntry hBox1
    entrySetText titleOptsEntry (gnuTitleOpts gnuParms)
    addWidgetToBox Nothing hBox1 contentBox

    hBox2 <- Gtk.boxNew Gtk.OrientationHorizontal 0
    xLabelEntry <- Gtk.entryNew
    addWidgetToBox (Just "X-label: ") xLabelEntry hBox2
    entrySetText xLabelEntry (gnuXLabel gnuParms)
    xLabelOptsEntry <- Gtk.entryNew
    addWidgetToBox Nothing xLabelOptsEntry hBox2
    entrySetText xLabelOptsEntry (gnuXLabelOpts gnuParms)
    addWidgetToBox Nothing hBox2 contentBox

    hBox3 <- Gtk.boxNew Gtk.OrientationHorizontal 0
    yLabelEntry <- Gtk.entryNew
    addWidgetToBox (Just "Y-label: ") yLabelEntry hBox3
    entrySetText yLabelEntry (gnuYLabel gnuParms)
    yLabelOptsEntry <- Gtk.entryNew
    addWidgetToBox Nothing yLabelOptsEntry hBox3
    entrySetText yLabelOptsEntry (gnuYLabelOpts gnuParms)
    addWidgetToBox Nothing hBox3 contentBox

    hBox4 <- Gtk.boxNew Gtk.OrientationHorizontal 0
    zLabelEntry <- Gtk.entryNew
    addWidgetToBox (Just "Z-label: ") zLabelEntry hBox4
    entrySetText zLabelEntry (gnuZLabel gnuParms)
    zLabelOptsEntry <- Gtk.entryNew
    addWidgetToBox Nothing zLabelOptsEntry hBox4
    entrySetText zLabelOptsEntry (gnuZLabelOpts gnuParms)
    addWidgetToBox Nothing hBox4 contentBox

    hBox5 <- Gtk.boxNew Gtk.OrientationHorizontal 0
    cbLabelEntry <- Gtk.entryNew
    addWidgetToBox (Just "CB-label: ") cbLabelEntry hBox5
    entrySetText cbLabelEntry (gnuCBLabel gnuParms)
    cbLabelOptsEntry <- Gtk.entryNew
    addWidgetToBox Nothing cbLabelOptsEntry hBox5
    entrySetText cbLabelOptsEntry (gnuCBLabelOpts gnuParms)
    addWidgetToBox Nothing hBox5 contentBox

    xTicsEntry <- Gtk.entryNew
    addWidgetToBox (Just "X-tics: ") xTicsEntry contentBox
    entrySetText xTicsEntry (gnuXTics gnuParms)

    yTicsEntry <- Gtk.entryNew
    addWidgetToBox (Just "Y-tics: ") yTicsEntry contentBox
    entrySetText yTicsEntry (gnuYTics gnuParms)

    zTicsEntry <- Gtk.entryNew
    addWidgetToBox (Just "Z-tics: ") zTicsEntry contentBox
    entrySetText zTicsEntry (gnuZTics gnuParms)

    cbTicsEntry <- Gtk.entryNew
    addWidgetToBox (Just "CB-tics: ") cbTicsEntry contentBox
    entrySetText cbTicsEntry (gnuCBTics gnuParms)

    -------------------------
    logScaleFrame <- Gtk.frameNew (Just "Log scale")
    addWidgetToBox Nothing logScaleFrame contentBox

    logScaleBox <- Gtk.boxNew Gtk.OrientationHorizontal 0
    Gtk.frameSetChild logScaleFrame (Just logScaleBox)

    logScaleXCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive logScaleXCheck (gnuLogScaleX gnuParms)
    addWidgetToBox (Just  "X:") logScaleXCheck logScaleBox

    logScaleYCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive logScaleYCheck (gnuLogScaleY gnuParms)
    addWidgetToBox (Just  "Y:") logScaleYCheck logScaleBox

    logScaleZCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive logScaleZCheck (gnuLogScaleZ gnuParms)
    addWidgetToBox (Just "Z:") logScaleZCheck logScaleBox
    -------------------------

    colorAndMapBox <- Gtk.boxNew Gtk.OrientationHorizontal 0
    addWidgetToBox Nothing colorAndMapBox contentBox

    paletteNegativeCheck <- Gtk.checkButtonNew
    addWidgetToBox (Just "Negative color: ") paletteNegativeCheck colorAndMapBox
    Gtk.checkButtonSetActive paletteNegativeCheck (gnuNegativePalette gnuParms)

    mapCheck <- Gtk.checkButtonNew
    addWidgetToBox (Just "Map: ") mapCheck colorAndMapBox
    Gtk.checkButtonSetActive mapCheck (gnuMap gnuParms)
    -------------------------

    gridCheck <- Gtk.checkButtonNew
    addWidgetToBox (Just "Grid: ") gridCheck contentBox
    Gtk.checkButtonSetActive gridCheck (gnuGrid gnuParms)

    borderEntry <- Gtk.entryNew
    addWidgetToBox (Just "Border: ") borderEntry contentBox
    entrySetText borderEntry (gnuBorder gnuParms)
    ...
    _ <- Gtk.onButtonClicked okButton $ do
        ...
        negativePalette <- Gtk.checkButtonGetActive paletteNegativeCheck
        viewAsMap <- Gtk.checkButtonGetActive mapCheck
        logScaleX <- Gtk.checkButtonGetActive logScaleXCheck
        logScaleY <- Gtk.checkButtonGetActive logScaleYCheck
        logScaleZ <- Gtk.checkButtonGetActive logScaleZCheck
        grid <- Gtk.checkButtonGetActive gridCheck
