...
localPhaseDialog stateRef = do
    ...
    periodAdjustment <- Gtk.adjustmentNew (localPhasePeriod parms) 0 (2**52) 1 1 1
    periodSpin <- Gtk.spinButtonNew (Just periodAdjustment) 1 10
    addWidgetToBox (Just "Period: ") periodSpin contentBox

    maxPeriodAdjustment <- Gtk.adjustmentNew (localPhaseMaxPeriod parms) 0 (2**52) 1 1 1
    maxPeriodSpin <- Gtk.spinButtonNew (Just maxPeriodAdjustment) 1 10
    addWidgetToBox (Just "Max period: ") maxPeriodSpin contentBox

    epochAdjustment <- Gtk.adjustmentNew (localPhaseEpoch parms) (-(2**52)) (2**52) 1 1 1
    epochSpin <- Gtk.spinButtonNew (Just epochAdjustment) 1 10
    addWidgetToBox (Just "Epoch: ") epochSpin contentBox

    precisionAdjustment <- Gtk.adjustmentNew (fromIntegral (localPhasePrecision parms)) 5 1000 1 1 1
    precisionSpin <- Gtk.spinButtonNew (Just precisionAdjustment) 1 0
    addWidgetToBox (Just "Precision: ") precisionSpin contentBox

    colorMapCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive colorMapCheck (localPhaseCalcColorMap parms)
    addWidgetToBox (Just "Calculate color map: ") colorMapCheck contentBox

    normalizeCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive normalizeCheck True
    addWidgetToBox (Just "Normalize: ") normalizeCheck contentBox

    barCodeCombo <- dataSetComboNew2 dataAndSpectrum state False
    addWidgetToBox (Just "Bar code from: ") (getComboBox barCodeCombo) contentBox

    phaseDispersionCheck <- Gtk.checkButtonNew
    Gtk.checkButtonSetActive phaseDispersionCheck (localPhaseCalcPhaseDisp parms)
    addWidgetToBox (Just "Calculate phase dispersion: ") phaseDispersionCheck contentBox

    avgOverCyclesAdjustment <- Gtk.adjustmentNew (fromIntegral (localPhaseAvgOverCycles parms)) 1 (2**52) 1 1 1
    avgOverCyclesSpin <- Gtk.spinButtonNew (Just avgOverCyclesAdjustment) 1 0
    addWidgetToBox (Just "Avg. over no. cycles: ") avgOverCyclesSpin contentBox
    ...
    _ <- Gtk.onButtonClicked okButton $ do
        ...
        calculateColorMap <- Gtk.checkButtonGetActive colorMapCheck
        normalize <- Gtk.checkButtonGetActive normalizeCheck
        barCode <- getSelectedData barCodeCombo
        calculatePhaseDispersion <- Gtk.checkButtonGetActive phaseDispersionCheck
